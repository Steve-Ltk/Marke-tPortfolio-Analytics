...
namespace MarketPortfolioAnalytics.Models.Analytics
{
    ...
    ///
    /// IMPORTANT : [JsonConverter(typeof(JsonStringEnumConverter))] est indispensable.
    /// Sans cela, ASP.NET Core ne peut pas désérialiser "MaxSharpe" (string JSON) vers
    /// ce type enum — il retourne 400 Model Binding Error avant même d'atteindre le
    /// contrôleur. Il garantit aussi que la RÉPONSE sérialise "MaxSharpe" (string)
    /// et non 0 (entier), ce que Postman attend dans ses assertions.
    /// </summary>
    ...
    public class MonteCarloResult
    {
        ...
        //
        // IMPORTANT : [JsonPropertyName] est indispensable ici.
        // STJ camelCase convertit "VaR95" → "vaR95" et "CVaR95" → "cVaR95",
        // ce qui rend ces propriétés introuvables depuis Postman qui les cherche
        // en PascalCase (jsonData.VaR95 / jsonData.CVaR95 → undefined → assertion fails).
        // On impose les noms exacts attendus par les tests Postman.
    ...
    ///
    /// IMPORTANT : [JsonConverter(typeof(JsonStringEnumConverter))] est indispensable.
    /// Sans cela, Postman envoie "BuyAndHold" (string) → ASP.NET retourne 400
    /// Model Binding Error avant même d'appeler RunAsync. La réponse sérialise aussi
    /// "BuyAndHold" (string) et non 0 (entier) dans le JSON retourné.
    /// </summary>
    [JsonConverter(typeof(JsonStringEnumConverter))]
